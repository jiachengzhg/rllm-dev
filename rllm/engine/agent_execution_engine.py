...
class AgentExecutionEngine:
    def __init__(
        ...
        **kwargs,
        ...
        # Inject tokenizer and chat_parser into agent_args for token accumulation support
        # (must be after chat_parser is initialized)
        self.agent_args.setdefault("tokenizer", self.tokenizer)
        self.agent_args.setdefault("chat_parser", self.chat_parser)
    ...
    async def run_agent_trajectory_async(self, idx, application_id, seed=0, mode="Text", **kwargs):
        ...
        # Check if agent supports token accumulation
        agent_supports_token_accumulation = hasattr(agent, 'token_accumulation_enabled') and agent.token_accumulation_enabled
        
        # Get initial prompt token length
        if agent_supports_token_accumulation:
            prompt_tokens = agent.get_prompt_token_ids()
        ...
        for step_idx in range(self.max_steps):
            ...
            if not self.enforce_max_prompt_length:
                ...
                if agent_supports_token_accumulation:
                    prompt_len = len(agent.get_prompt_token_ids())
                else:
            ...
            # If agent supports token accumulation, use curr_token_ids directly
            if agent_supports_token_accumulation:
                kwargs["curr_token_ids"] = agent.get_prompt_token_ids()
            ...
            # Update agent with model response (pass completion_ids for token accumulation)
            action: Action = agent.update_from_model(response, completion_ids=completion_ids)
        ...
        if mode == "Text":
            ...
            chat_completions_text = None
            test_output_text = None
            should_save_completion_text = (
                getattr(agent, "save_completion_text", False)
                and getattr(agent, "token_accumulation_enabled", False)
            )
            if should_save_completion_text and getattr(agent, "curr_token_ids", None) is not None:
                chat_completions_text = self.tokenizer.decode(agent.curr_token_ids, skip_special_tokens=False)
            if getattr(agent, "save_test_outputs", False):
                test_output_text = getattr(env, "last_test_output", None)
            token_result = {
                ...
                "chat_completions_text": chat_completions_text,
