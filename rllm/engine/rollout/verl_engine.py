...
class VerlEngine(RolloutEngine):
    ...
    async def get_model_response(self, messages: list[dict], **kwargs) -> ModelOutput:
        ...
        # Token accumulation support: if curr_token_ids is provided, use it directly
        # instead of parsing messages (avoids retokenization issues)
        curr_token_ids = kwargs.pop("curr_token_ids", None)
        ...
        if curr_token_ids is not None:
            # Use provided token IDs directly (token accumulation mode)
            request_prompt_ids = curr_token_ids
            prompt_ids = curr_token_ids
            image_data = None
            multi_modal_inputs = None
        else:
            # Traditional mode: parse messages and encode
