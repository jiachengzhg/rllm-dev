...
class AgentPPOTrainer(RayPPOTrainer):
    ...
    def _transform_agent_trajectories(self, trajectories: list[dict]):
        ...
        for traj in trajectories:
            ...
            chat_completions_text.append(traj.get("chat_completions_text"))
            test_outputs.append(traj.get("test_output_text"))
        ...
        # Save per-sample decoded completion texts if available.
        if any(text is not None for text in chat_completions_text):
            save_text_dir = os.path.join(self.config.trainer.default_local_dir, "chat_completions_text", f"{self.global_steps}")
            os.makedirs(save_text_dir, exist_ok=True)
            for i, completion_text in enumerate(chat_completions_text, start=1):
                with open(os.path.join(save_text_dir, f"{i}.txt"), "w", encoding="utf-8") as f:
                    f.write("" if completion_text is None else completion_text)

        # Save per-sample reward test outputs if available.
        if any(text is not None for text in test_outputs):
            save_test_output_dir = os.path.join(self.config.trainer.default_local_dir, "test_outputs", f"{self.global_steps}")
            os.makedirs(save_test_output_dir, exist_ok=True)
            for i, test_output in enumerate(test_outputs, start=1):
                with open(os.path.join(save_test_output_dir, f"{i}.txt"), "w", encoding="utf-8") as f:
                    f.write("" if test_output is None else test_output)
